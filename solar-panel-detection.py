...
'''
INSERT THE CODE SNIPPET FOR IMAGES WITH SOLAR PANELS HERE
'''
ones = X[y == 1]  # Subset of images with solar panels
i_ones = random.sample(range(len(ones)), 25)  # Randomly select 25 images

subprocess.Popen(["gnome-terminal", "--", "python", "-c", "import matplotlib.pyplot as plt; plt.imshow(ones[i_ones[0]]); plt.axis('off'); plt.show()"])

'''
INSERT THE CODE SNIPPET FOR IMAGES WITH NO SOLAR PANELS HERE
'''
zeros = X[y == 0]  # Subset of images with no solar panels
i_zeros = random.sample(range(len(zeros)), 25)  # Randomly select 25 images

subprocess.Popen(["gnome-terminal", "--", "python", "-c", "import matplotlib.pyplot as plt; plt.imshow(zeros[i_zeros[0]]); plt.axis('off'); plt.show()"])

'''
INSERT THE CODE SNIPPET FOR DISPLAYING THE TABLE HERE
'''
data = [["With Solar Panel", len(ones)], ["Without Solar Panel", len(zeros)], ["Total", len(X)]]
...
def preprocess_and_extract_features(data):
    ...
    for row in data:
        ...
        fd = hog(resized_img, orientations=9, pixels_per_cell=(20, 20),
                 cells_per_block=(5, 5), visualize=False, channel_axis=-1)
        fd_all.append(fd)
        pass
